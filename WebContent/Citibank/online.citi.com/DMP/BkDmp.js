...
var bk = {
	...
	sendDmpUpdates: function() {
		$.ajax({
			...
			type : "POST",
			data: "token=" + parsed_bk_result_format + "&siteCatalystId=" + getCookie("s_vi") + "&CUUID=" + getCookie("CUUID"),
			dataType : "script",
		...
		$.ajax({
			...
			type : "POST",
			data: "token=" + parsed_bk_result_format + "&CUUID=" + getCookie("CUUID"),
			dataType : "script",
			success : function (data, status, jqxHR) {
			}
		});
	    ...
	    $.ajax({
			...
			success : function (data, status, jqxHR) {
		    	if (typeof bk_results == 'undefined') {
		        	...
		        	bk.parseBkData();
		        	bk.createCookie("BKDMP", parsed_bk_result_format, 1);
		        	bk.updateDmpCache();
		            //bk.sendDmpUpdates();
		        }
			},
