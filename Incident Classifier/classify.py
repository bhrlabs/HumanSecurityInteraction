...
classifier2 = DecisionTreeClassifier(training_data)
print "Classifier Ready"

print 'NaiveBayesClassifier Accuracy - ', classifier.accuracy(test_data)
print 'DecisionTreeClassifier Accuracy - ', classifier2.accuracy(test_data)
...
for key, row in test_data_raw.iterrows():
   ...
   unlabel_data.append({'Incident': item,  'Classification' : classification})

"""
with open('Classifier_results2.csv', 'wb') as f:
   w = csv.DictWriter(f, fieldnames=['Incident', 'Classification'])
   w.writeheader()
   for row in unlabel_data:
       w.writerow(row)
"""
print "Exit"
